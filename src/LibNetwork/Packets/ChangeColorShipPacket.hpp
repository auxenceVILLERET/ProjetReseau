#ifndef CHANGE_COLOR_SHIP_PACKET_HPP_DEFINED
#define CHANGE_COLOR_SHIP_PACKET_HPP_DEFINED

#include "Packet.h"

class ChangeColorShipPacket: public Packet
{
	public:
	uint32_t id;
	int index;
	ChangeColorShipPacket()
	{
		id = 0;
		index = 0;
		m_type = CHANGE_COLOR_SHIP;
		m_size = 2 * sizeof(int) + sizeof(id) + sizeof(index);
	}
	ChangeColorShipPacket(uint32_t _id, int _index)
	{
		id = _id;
		index = _index;
		m_type = CHANGE_COLOR_SHIP;
